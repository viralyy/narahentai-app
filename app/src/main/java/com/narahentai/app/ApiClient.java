...
public class ApiClient {
    ...
    public static List<VideoItem> fetchPosts(String apiUrl) throws Exception {
        ...
        for (int i = 0; i < items.length(); i++) {
            ...
            String publishedAt = o.optString("published_at", o.optString("created_at", ""));

            list.add(new VideoItem(title, slug, thumb, duration, views, publishedAt));
        }
        return list;
    }

    // ambil detail post untuk dapet video_url (mp4)
    public static JSONObject fetchPostDetail(String apiUrl) throws Exception {
        String json = httpGet(apiUrl);
